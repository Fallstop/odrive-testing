...
CurrLimBox =Entry(window,width=10)
CurrLimBox.grid(column = 2, row = 13)
...
def setCalbttn():
    ...
    CurrLimVal.configure(text =  my_drive.axis0.motor.config.current_lim)
...
def writeCurrLim():
    global my_drive
    CurrLim = float(CurrLimBox.get())
    my_drive.axis0.motor.config.current_lim = CurrLim
    CurrLimVal.configure(text =  my_drive.axis0.motor.config.current_lim)
...
wrtCurrLim = Button(window, text="Write new current_lim", command=writeCurrLim)
wrtCurrLim.grid(column =3,row =13)
